...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            f'saree_processing_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
        ),
...
class Pattern(str, Enum):
    ...
    JACQUARD = "Jacquard"
    DHOOP_CHAON = "Dhoop Chaon"
    SCALLOPPED = "Scallopped"
    AJRAK = "Ajrak"
    HANDLOOM = "Handloom"
...
# ---------- Pydantic Models ---------- 
class SareeAttributes(BaseModel):
    ...
    def validate_saree_type(cls, v: Any) -> SareeType:
        """Validate saree_type with smart matching."""
        ...
        if isinstance(v, str):
            ...
            if best_match and score >= 0.8:
                logger.info(f"Fuzzy matched saree_type '{v}' to '{best_match.value}' with score {score}")
                return SareeType(best_match.value)
        
        logger.warning(f"Could not match saree_type '{v}', defaulting to 'Others'")
        return SareeType.OTHERS

    @field_validator('primary_color', mode='before')
    def validate_primary_color(cls, v: Any) -> Color:
        """Validate primary_color with smart matching."""
    ...
    def validate_primary_color_detailed(cls, v: Any) -> ColorDetailed:
        """Validate primary_color_detailed with smart matching."""
    ...
    def validate_secondary_colors(cls, v: Any) -> List[Color]:
        """Validate secondary_colors with smart matching."""
        if not v:
            return []
        ...
        for color in v:
            if isinstance(color, Color):
                ...
                logger.warning(f"Invalid secondary color type, defaulting to 'Others'")
                validated_colors.append(Color.OTHERS)
    ...
    def validate_secondary_colors_detailed(cls, v: Any) -> List[ColorDetailed]:
        """Validate secondary_colors_detailed with smart matching."""
        if not v:
            return []
    ...
    def validate_blouse_material(cls, v: Any) -> Optional[Material]:
        ...
        if isinstance(v, str):
            ...
            best_match, score = find_best_enum_match(v, Material)
            if best_match and score >= 0.8:
                logger.info(f"Fuzzy matched blouse_material '{v}' to '{best_match.value}' with score {score}")
                return Material(best_match.value)
            ...
            if len(words) > 1:
                for word in words:
                    ...
                    if match and score > 0.9:
                        logger.info(f"Matched compound blouse_material word '{word}' from '{v}' to '{match.value}'")
                        return Material(match.value)
        
        logger.warning(f"Could not match blouse_material '{v}', defaulting to None")
        return None

    @field_validator('pattern', mode='before')  # Add mode='before'
    def validate_pattern(cls, v: Any) -> List[Pattern]:
        """Validate pattern with smart enum matching and default to 'Others'."""
        if not isinstance(v, list):
            v = [v]
        ...
        for pattern in v:
            ...
            if isinstance(pattern, str):
                ...
                logger.warning(f"Invalid pattern type → defaulting to 'Others'")
                validated_patterns.append(Pattern.OTHERS)
    ...
    def validate_border_width(cls, v: Any) -> BorderWidth:
        """Validate border_width with smart matching."""
        ...
        if isinstance(v, str):
            ...
            if best_match and score >= 0.8:
                logger.info(f"Fuzzy matched border_width '{v}' to '{best_match.value}' with score {score}")
                return BorderWidth(best_match.value)
        
                return best_match
        
        logger.warning(f"Could not match border_width '{v}', defaulting to 'Others'")
        return BorderWidth.OTHERS

    @field_validator('border_design', mode='before')
    def validate_border_design(cls, v: Any) -> BorderDesign:
        """Validate border_design with smart matching."""
        ...
        if isinstance(v, str):
            ...
            if best_match and score >= 0.8:
                logger.info(f"Fuzzy matched border_design '{v}' to '{best_match.value}' with score {score}")
                return best_match
        
        logger.warning(f"Could not match border_design '{v}', defaulting to 'Others'")
        return BorderDesign.OTHERS

    @field_validator('pallu_design', mode='before')
    def validate_pallu_design(cls, v: Any) -> PalluDesign:
        """Validate pallu_design with smart matching."""
        ...
        if isinstance(v, str):
            ...
            if best_match and score >= 0.8:
                logger.info(f"Fuzzy matched pallu_design '{v}' to '{best_match.value}' with score {score}")
                return best_match
        
        logger.warning(f"Could not match pallu_design '{v}', defaulting to 'Others'")
        return PalluDesign.OTHERS

    @field_validator('occasions', mode='before')
    def validate_occasions(cls, v: Any) -> List[Occasion]:
        """Validate occasions with smart matching."""
        if not v:
        ...
        for occasion in v:
            ...
            if isinstance(occasion, str):
                # Try exact match first
                try:
                    validated_occasions.append(Occasion(occasion))
                    continue
                except ValueError:
                    pass
                ...
                if best_match and score >= 0.8:
                    ...
                    logger.warning(f"Could not match occasion '{occasion}', defaulting to 'Other'")
                    validated_occasions.append(Occasion.OTHER)
            else:
                logger.warning(f"Invalid occasion type, defaulting to 'Other'")
                validated_occasions.append(Occasion.OTHER)
                
        return validated_occasions or [Occasion.OTHER]

    @field_validator('embellishment_level', mode='before')
    def validate_embellishment_level(cls, v: Any) -> EmbellishmentLevel:
        """Validate embellishment_level with smart matching."""
    ...
    def validate_embellishment(cls, v: Any) -> List[Embellishment]:
        """Validate embellishment with smart matching."""
        if not v:
    ...
    def validate_style(cls, v: Any) -> List[Style]:
        """Validate style with smart matching."""
        if not v:
            return [Style.OTHERS]
        ...
        return validated_styles or [Style.OTHERS]

    @field_validator('gender', mode='before')
    def validate_gender(cls, v: Any) -> List[Gender]:
        """Validate gender with smart matching."""
        if not v:
            return [Gender.WOMEN]
    ...
    def validate_age_group(cls, v: Any) -> List[AgeGroup]:
        """Validate age_group with smart matching."""
        if not v:
        ...
        for age_group in v:
            if isinstance(age_group, AgeGroup):
                ...
                logger.warning(f"Invalid age_group type, defaulting to 'Adult'")
                validated_age_groups.append(AgeGroup.ADULT)
...
ENUM_MAPPINGS = {
    ...
    for enum_cls in [
        Color, ColorDetailed, Pattern, Material, SareeType, BorderWidth,
        BorderDesign, PalluDesign, Occasion,
        Style, EmbellishmentLevel, Embellishment, Gender, AgeGroup
    ]
}
...
class IncrementalSaver:
    ...
    def _prepare_ordered_result(self, product_id: str, result: dict) -> dict:
        ...
        # Core attributes
        ordered_result['saree_type'] = result.get('saree_type', '')
        ...
        # Design elements
        ordered_result['pattern'] = result.get('pattern', [])
        ordered_result['border_width'] = result.get('border_width', '')
        ordered_result['border_design'] = result.get('border_design', '')
        ordered_result['border_design_details'] = result.get('border_design_details', [])
        ordered_result['pallu_design'] = result.get('pallu_design', '')
        ...
        # Physical attributes
        ordered_result['length'] = result.get('length', 0)
        ordered_result['width'] = result.get('width', 0)
        ordered_result['weight'] = result.get('weight', 0)
        ordered_result['pre_draped'] = result.get('pre_draped', False)
        ordered_result['reversible'] = result.get('reversible', False)
        
        # Blouse information
        ordered_result['blouse_included'] = result.get('blouse_included', False)
        ordered_result['blouse_material'] = result.get('blouse_material', None)
    ...
    def _prepare_row(self, result: dict) -> dict:
        ...
        if result:
            ...
            attributes = {
                'saree_type', 'primary_color', 'primary_color_detailed', 'primary_color_hex',
                'secondary_colors', 'secondary_color_hexes', 'secondary_colors_detailed',
                'pattern', 'border_width', 'border_design', 'border_design_details',
                'pallu_design', 'pre_draped', 'material', 'reversible',
                'length', 'width', 'weight', 'occasions', 'occasion_detailed',
                'embellishment_level', 'embellishment', 'embellishment_detailed',
                'style', 'brand', 'brand_title', 'title', 'description', 'price',
                'care_instructions', 'blouse_included', 'blouse_material', 'size',
                'unique_design_element', 'search_context'
            }
...
async def main():
    ...
    saver = IncrementalSaver(
        json_path='/Users/aswinsreenivas/Projects/D2C Search Engine/uplyft_server/prodcut_desc_claude/saree_attributes/saree_attributes_results.json',
        excel_path='/Users/aswinsreenivas/Projects/D2C Search Engine/uplyft_server/prodcut_desc_claude/saree_attributes/saree_attributes_results.xlsx'
    )
    
    # Create output directories
    os.makedirs('/Users/aswinsreenivas/Projects/D2C Search Engine/uplyft_server/prodcut_desc_claude/saree_attributes', exist_ok=True)
    os.makedirs('/Users/aswinsreenivas/Projects/D2C Search Engine/uplyft_server/prodcut_desc_claude/saree_attributes/images', exist_ok=True)
    
    try:
        print("\nStarting saree attribute extraction...")
        ...
        df = df[
            df['Product Type'].str.contains('saree', case=False, na=False)
        ].copy()
        ...
        print(f"\nFound {len(df)} new saree products to process")
        
        if len(df) == 0:
            print("No new saree products found to process")
            return pd.DataFrame()
...
def split_attributes(data: dict) -> dict:
    ...
    non_enum_fields = [
        ...
        'secondary_colors_detailed_hex', 'border_design_details',
        'reversible', 'length', 'width', 'weight', 'occasion_detailed',
        'embellishment_detailed', 'coordinating_items', 'color_pairings',
        'brand', 'brand_title', 'title', 'description', 'price',
        'care_instructions', 'blouse_included', 'blouse_material',
        'size', 'unique_design_element'
    ]
...
def get_product_attributes(product_type: str) -> dict:
    attribute_instructions = {
        "inferred_attributes": {
            ...
            for k, v, e in [
                ("saree_type", "Type of saree", SareeType),
                ...
                ("border_width", "Border width", BorderWidth),
                ("border_design", "Border design", BorderDesign),
                ("pallu_design", "Pallu design", PalluDesign),
                ("pre_draped", "Draping style", bool),
                ("material", "Fabric material", Material),
                ("occasions", "Suitable occasions", Occasion),
                ...
                ("reversible", "Whether reversible", bool)
            ]
        },
        "given_attributes": {
            ...
            "length": "Length in meters (4.5-9.5)",
            "width": "Width in meters (0.9-1.4)",
            "weight": "Weight in grams (300-1000)",
            "blouse_included": "True/False",
            "blouse_material": f"Material from {', '.join(Material.__members__.keys())}",
...
SYSTEM_PROMPT = f"""You are a fashion expert specializing in Indian ethnic wear, particularly sarees. 
        ...
        <schema>{SareeAttributes.schema_json(indent=2)}</schema>
...
async def process_product_internal(row: dict) -> tuple:
    try:
        ...
        output_image_path = os.path.join(
            '/Users/aswinsreenivas/Projects/D2C Search Engine/uplyft_server/prodcut_desc_claude/saree_attributes/images',
            f"{product_id}.jpg"
        )
        try:
        ...
        try:
            ...
            saree_attrs = SareeAttributes(**result)
            ...
            try:
                embedding = call_cohere_api(
                    COHERE_CLIENT,
                    'embed',
                    texts=[saree_attrs.search_context],
                    model="embed-english-v3.0",
