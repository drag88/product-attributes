...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            f'kurta_processing_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
        ),
...
class Closure(str, Enum):
    ...
    TIE_BACK = "Tie-back closure"
    SIDE_ZIPPER = "Side Zipper"
    PULL_OVER = "Pull over/Slip on"
...
# ---------- Pydantic Model ----------
class KurtaAttributes(BaseModel):
    primary_color: Color
    ...
    def validate_length(cls, v: Any) -> Length:
        ...
        if isinstance(v, str):
            ...
            best_match, score = find_best_enum_match(v, Length)
            if best_match and score >= 0.8:
                logger.info(f"Fuzzy matched length '{v}' to '{best_match.value}' with score {score}")
                return Length(best_match.value)
        
        logger.warning(f"Could not match length '{v}', defaulting to 'Others'")
        return Length.OTHERS
    ...
    def validate_kurta_set(cls, v: Any) -> KurtaSet:
        ...
        if isinstance(v, str):
            ...
            if best_match and score >= 0.8:
                logger.info(f"Fuzzy matched kurta_set '{v}' to '{best_match.value}' with score {score}")
                return KurtaSet(best_match.value)
        
        logger.warning(f"Could not match kurta_set '{v}', defaulting to 'Others'")
        return KurtaSet.OTHERS
...
ENUM_MAPPINGS = {
    ...
    for enum_cls in [
        ...
        Occasion, SleeveType, Style, EmbellishmentLevel,
        Embellishment, Gender, AgeGroup, Length, KurtaSet
    ]
}
...
class IncrementalSaver:
    ...
    def _prepare_ordered_result(self, product_id: str, result: dict) -> dict:
        ...
        ordered_result['kurta_set'] = result.get('kurta_set', '')
        ordered_result['length'] = result.get('length', '')
        ...
        ordered_result['product_type'] = 'kurta'
    ...
    def _prepare_row(self, result: dict) -> dict:
        ...
        if result:
            ...
            attributes = {
                ...
                'pattern', 'material', 'neckline', 'sleeve_type', 'closure', 'length',
                'fit', 'hemline', 'kurta_set',
...
async def main():
    ...
    saver = IncrementalSaver(
        json_path='/Users/aswinsreenivas/Projects/D2C Search Engine/uplyft_server/prodcut_desc_claude/kurta_attributes/kurta_attributes_results.json',
        excel_path='/Users/aswinsreenivas/Projects/D2C Search Engine/uplyft_server/prodcut_desc_claude/kurta_attributes/kurta_attributes_results.xlsx'
    )
    
    # Create output directories
    os.makedirs('/Users/aswinsreenivas/Projects/D2C Search Engine/uplyft_server/prodcut_desc_claude/kurta_attributes', exist_ok=True)
    os.makedirs('/Users/aswinsreenivas/Projects/D2C Search Engine/uplyft_server/prodcut_desc_claude/kurta_attributes/images', exist_ok=True)
    
    try:
        print("\nStarting kurta attribute extraction...")
        ...
        df = df[
            df['Product Type'].str.contains('Kurta', case=False, na=False)
        ].copy()
        ...
        print(f"\nFound {len(df)} new kurta products to process")

        if len(df) == 0:
            print("No new kurta products found to process")
            return pd.DataFrame()
    ...
    attribute_instructions = {
        "inferred_attributes": {
            ...
            for k, v, e in [
                ...
                ("length", "Length of the product", Length),
...
SYSTEM_PROMPT = f"""You are a fashion expert specializing in Indian ethnic wear, particularly for kurta. 
        Analyze this kurta image and product information to generate detailed JSON data.
        
        IMPORTANT GUIDELINES:
        - Focus ONLY on the kurta in the image, not any other garments shown
        - Use the exact product name from the data for brand_title (e.g. if product name is "Green Dream", use that)
        - Analyze colors ONLY from the kurta itself, not from any accompanying garments or accessories
        - Be precise in identifying embellishments and patterns that are actually present on the kurta
        ...
        Requirements:
        1. Colors & Patterns:
           - "primary_color": Identify and list the single most dominant color in the product as primary_color. primary_color MUST be one of: {', '.join(Color.__members__.keys())} (ONLY for the kurta). 
           - "primary_color_hex": Provide the exact hex code for the primary color (e.g., #FF5733)
           - "primary_color_detailed": Identify and list the single most dominant color in the product as primary_color_detailed. primary_color_detailed MUST be one of: ColorDetailed (ONLY for the kurta)
           - "secondary_colors": Identify and list the next most prominent colors in the product, secondary_colors MUST be one of: {', '.join(Color.__members__.keys())} and ONLY for the kurta
           ...
           - "secondary_colors_detailed": Identify and list the next most prominent colors from ONLY the kurta, if any in a comma-separated python list format. For each secondary color, provide a closest matching secondary_colors_detailed from ColorDetailed enum
           ...
           - "pattern": Describe the patterns only in kurta in a comma-separated python list format. pattern MUST ONLY use these values: {', '.join(Pattern.__members__.keys())} (based on patterns in kurta ONLY). Make sure to include only patterns and not embellishments that are actually present in the kurta.
        ...
        2. Design Elements:
           ...
           - "length": Must be one of: {', '.join(Length.__members__.keys())}
        ...
        3. Material & Structure:
           ...
           - "kurta_set": Must be one of: {', '.join(KurtaSet.__members__.keys())}
        ...
        4. Usage & Style:
           ...
           - embellishment_detailed: embellishment_detailed MUST be a list of 3-5 detailed embellishments present in the kurta and only if the kurta has any embellishments. Detail any decorative elements or embellishments on the product in a comma-separated python list format for embellishment_detailed
        ...
        6. Coordinating Items:
           - "clothing": REQUIRED - Suggest EXACTLY 5 coordinate clothing items that would pair well with the main product. Consider:
             * Traditional options (e.g., kurtas, cholis)
             * Contemporary options (e.g., crop tops, jackets)
             * Color coordination with the kurta's primary and secondary colors
        ...
        8. Unique Selling Points:
           - unique_design_element: List 1-3 distinct selling points that make this item unique. Focus on:
             ...
             * Versatile styling (e.g., "Reversible design offers two looks in one kurta")
        ...
        <schema>{KurtaAttributes.schema_json(indent=2)}</schema>
...
async def process_product_internal(row: dict) -> tuple:
    try:
        ...
        output_image_path = os.path.join(
            '/Users/aswinsreenivas/Projects/D2C Search Engine/uplyft_server/prodcut_desc_claude/kurta_attributes/images',
            f"{product_id}.jpg"
        )
        try:
        ...
        try:
            ...
            kurta_attrs = KurtaAttributes(**result)
            ...
            try:
                embedding = call_cohere_api(
                    COHERE_CLIENT,
                    'embed',
                    texts=[kurta_attrs.search_context],
                    model="embed-english-v3.0",
