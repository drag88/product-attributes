...
class Trouser(ClothingItem):
    _config = ConfigManager.get_merged_config('trouser')
    ...
    def __init_subclass__(cls):
        ...
        for field_name, config in cls._attr_config.items():
            ...
            elif config.get('item_type') == 'list':
                field_type = List[str]
                field_default = []
                if 'min_length' in config:
                    field_kwargs['min_length'] = config['min_length']
    ...
    def validate_attributes(self) -> List[str]:
        ...
        # Validate fit and style combinations
        if (enum_fields.get('fit') == 
                self._get_enum_class('TrouserFit').SKINNY and 
                enum_fields.get('style') == 
                self._get_enum_class('TrouserStyle').BAGGY):
            errors.append("Skinny fit trousers cannot have baggy style")
            
        # Validate rise and fit combinations
        if (enum_fields.get('rise') == 
                self._get_enum_class('TrouserRise').LOW and 
                enum_fields.get('fit') == 
                self._get_enum_class('TrouserFit').LOOSE):
            errors.append("Low rise trousers should not have loose fit")
