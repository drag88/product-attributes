...
class Lehenga(ClothingItem):
    _config = ConfigManager.get_merged_config('lehenga')
    ...
    def validate_attributes(self) -> List[str]:
        ...
        # Validate blouse and dupatta inclusion
        if not self.blouse_included and enum_fields.get('blouse_design'):
            errors.append("Blouse design specified but blouse is not included")
            
        if not self.dupatta_included and enum_fields.get('dupatta_style'):
            errors.append("Dupatta style specified but dupatta is not included")
